...
check_url_once() {
    ...
    content=$(curl -s -L --max-time 10 -A "$UA" "$url" 2>/dev/null)
    [[ -z "$content" ]] && return 1
    ...
    if echo "$content" | head -5 | grep -qE '#EXT-X-STREAM-INF'; then
        ...
        content=$(curl -s -L --max-time 10 -A "$UA" "$variant" 2>/dev/null)
        [[ -z "$content" ]] && return 1
    fi
    ...
    code=$(curl -s -o /dev/null -w "%{http_code}" -L --max-time 10 -A "$UA" "$seg" 2>/dev/null)
